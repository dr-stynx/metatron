...
public class fsInstSet extends MInstSet {
    ...
    public static final fURI DIR_TID = FS_INSTSET_TID.extend("dir");
    ...
    public Set<Type> types() {
        return Set.of(
                T(IMAGE_TID, isa_(T(STR_TID))),
                T(FILE_TID, isa_(rec(uri(PATTERN), T(URI_TID))), instC(INST_TID.dom(ALL.maybe()).rng(FILE_TID), lst(T(REC_TID)), (lhs, inst) -> fileSpace.makeFile(Path.of(inst.arg(0).<Rec>as().at(PATTERN).uriValue().toString())))));
    ...
    public Set<Inst> insts() {
        return new LinkedHashSet<>(List.of(
                ...
                instC(FS_INSTSET_TID.extend("convert").dom(BYTES_TID).rng(IMAGE_TID), lst(T(IMAGE_TID)), (lhs, inst) -> str(ImageFile.convertToAscii(lhs.bytesValue())))));
    }
}