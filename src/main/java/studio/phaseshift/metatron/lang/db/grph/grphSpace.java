...
public class grphSpace extends MSpace<Space> {
    ...
    public static final Type GRPH_TYPE = T(GRPH_TID, null, instC(mInstSet.INST_TID.dom(ALL.maybe()).rng(GRPH_TID),
            lst(isa_(rec(uri(PATTERN), T(URI_TID),
                ...
                final fURI dataset = inst.arg(0).<Rec>as().at(LOAD).orElse(uri("")).uriValue();
                final Space store = inst.arg(0).<Rec>as().at(STORE).as();
                final grphSpace space = new grphSpace(store.isNoObj() ? noobjSpace.single() :
                        new kvSpace(store.<Rec>as().at(PATTERN).uriValue(), fURI.fnull), Map.of(uri(PATTERN), uri(pattern), uri(LOAD), uri(dataset)), pattern, inst.arg(0).vid());
