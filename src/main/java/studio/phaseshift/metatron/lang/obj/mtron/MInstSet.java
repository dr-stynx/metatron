...
public class MInstSet extends MSpace implements InstSet {
    ...
    public void load() {
        ...
        this.write(
                ...
                PLUS_TID, instC(PLUS_TID.dom(REC_TID).rng(REC_TID), lst(isA(T(REC_TID))), (lhs, inst) -> lhs.value(Stream.concat(lhs.recValue().entrySet().stream(), inst.arg(0).recValue().entrySet().stream()).collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue, (a, b) -> b,LinkedHashMap::new)))),
