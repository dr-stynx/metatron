...
public interface Code extends Call {
    ...
    @Override
    default Type dom() {
        return this.value().isEmpty() ? T(fURI.NONE.zero()) : T(this.value().get(0).dom().tid().all()); // TODO: if unresolved, it's maybe.. is that good?
    }

    default Type rng() {
        return this.value().isEmpty() ? T(fURI.NONE.zero()) : T(this.value().get(this.value().size() - 1).rng().tid().all());
    }
