...
public class mDocs {
    ...
    public static void attach(final Space space) {
        ...
        List.of(
                        DocQ.Doc.doc(Router.readFromSpace(CHOOSE_TID).as(), "any obj", "the split as an objs", Map.of(f("0"), "the branches"), "a branching inst"),
                        DocQ.Doc.doc(Router.readFromSpace(IS_TID).as(), "any obj", "the lhs obj if arg is true", Map.of(f("0"), "filter lhs if false"), "filters the lhs obj"))
                .forEach(doc -> {
