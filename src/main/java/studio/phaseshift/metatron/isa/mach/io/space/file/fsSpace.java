...
public class fsSpace extends AbstractSpace<FileSystem> {
    ...
    public Obj resolveObj(final Uri path) {
        try {
            ...
            return uri(this.rewrite(fURI.f(file.getPath()), false).q("p", PosixFilePermissions.toString(Files.getPosixFilePermissions(file.toPath()))), file.isDirectory() ? DIR_TID : FILE_TID, null);
        } catch (final Exception e) {
            throw MTronException.of(e);
        }
    }
