            ...
            return new LinkedHashSet<>(List.of(
                  ...
                  /*  instC(REPEAT_INST_TID.dom(A).rng(A.maybeSome()).query(MONAD, null), lst(T(ALL), T(ALL)), (lhs, inst) -> {
                    ...
                    docWrap(instC(AND_INST_TID.dom(A).rng(BOOL_TID), lst(T(BOOL_TID).c(cInt::some)), (lhs, inst) -> bool(inst.args().elements().map(Obj::asBool).allMatch(Obj::boolValue))),
                            "any objs", "true if all objs are true", Map.of(), "a logical and function \\(f(X)\\to \\tt{true}\\) if all \\(X\\) are true"),
                    docWrap(instC(OR_INST_TID.dom(A).rng(BOOL_TID), lst(T(BOOL_TID).c(cInt::some)), (lhs, inst) -> bool(inst.args().elements().map(Obj::asBool).anyMatch(Obj::boolValue))),
                            "any objs", "true if any objs are true", Map.of(), "a logical or function \\(f(X)\\to \\tt{true}\\) if any \\(X\\) are true"),
                    ...
                    docWrap(instC(ELSE_INST_TID.dom(ALL.maybe()).rng(ALL), lst(T(ALL.maybe())), (lhs, inst) -> lhs.isNoObj() ? inst.arg(0) : lhs),
                            "maybe an obj", "the lhs obj else the arg obj", Map.of(jnt(0), "the rhs obj is the lhs is noobj"), "\\[ f(x)(a) = \\left\\{ \\begin{aligned} \\tt{true} & \\quad \\text{if } x \\in \\tt{obj} \\\\ \\tt{arg}_0 & \\quad \\text{otherwise} \\end{aligned} \\right. \\]"),// TODO: rec args needs resolution on generics connected
                    ...
                    docWrap(instC(EQ_INST_TID.dom(ALL).rng(BOOL_TID), lst(T(ALL)), (lhs, inst) -> Inst.Helper.alignLHSType(lhs, inst.arg(0)).map(l -> Objects.equals(l, inst.arg(0))).map(MBool::bool).orElse(BOOL_FALSE)),
                            "any objs", "true if lhs equals rhs", Map.of(jnt(0), "the rhs obj"), "an equality function \\(f(x,y)\\to \\tt{true} if \\(x \\eq y\\)"),
                    docWrap(instC(NEQ_INST_TID.dom(ALL).rng(BOOL_TID), lst(T(ALL)), (lhs, inst) -> Inst.Helper.alignLHSType(lhs, inst.arg(0)).map(l -> !Objects.equals(l, inst.arg(0))).map(MBool::bool).orElse(BOOL_TRUE)),
                            "any objs", "true if lhs does not equal rhs", Map.of(jnt(0), "the rhs obj"), "an inequality function \\(f(x,y)\\to \\tt{true}\\) if \\(x \\neq y\\)"),
