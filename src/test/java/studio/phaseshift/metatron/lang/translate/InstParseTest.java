...
public class InstParseTest extends MetatronTest {
    ...
    @CsvSource(value = {
           ...
           // "|(/usr/abc?int<=int(int::T[],int::T[]){ mult(*<0>).plus(*<1>) }).to(/usr/abc)               % 2./usr/abc(_,_)                % 6",
            ...
            //"|(/usr/abc?int<=int(int::T[],lst::T[]){ mult(*<0>).plus(*<1>>-) }).to(/usr/abc)               % 2./usr/abc(3,[4])                % 10",
            //"|(/usr/abc?int<=int(int::T[],int::T[]){ mult(*<0>).plus(*<1>) }).to(/usr/abc)               % 2./usr/abc(3,4)                % 10",
    ...
    void testInstDefinitions(final String definition, final String usage, final String expected) {
        Call def = mtronParser.eval(definition);
        Obj use = mtronParser.eval(usage);
    ...
    void testPlusInst(final String expression, final String expectedResult) {
        assertEquals(mtronParser.m_obj().parse(expectedResult).get(), mtronParser.<Obj>eval(expression));
    }
