...
public class MServerTest extends AbstractMetatronTest {
    ...
    /**
     * Test MCP evaluate_code tool.
     */
    @Test
    @Order(21)
    @TestCategory.Crud
    public void testMcpEvaluateCodeTool() throws Exception {
        final CountDownLatch initLatch = new CountDownLatch(1);
        final CountDownLatch toolLatch = new CountDownLatch(1);
        final AtomicReference<String> toolResponse = new AtomicReference<>();
        ...
        final WebSocketClient client = new WebSocketClient(URI.create(serverHost.toString())) {
            ...
            protected void sendInitialize() {
                ...
                try {
                    ...
                    LOG.error("Failed to send initialize: %s", e.getMessage());
                }
            }

            protected void sendToolCall() {
                final Map<String, Object> request = new HashMap<>();
                request.put("jsonrpc", "2.0");
                request.put("id", 2);
                ...
                request.put("params", Map.of(
                        "name", "evaluate_code",
                        "arguments", Map.of("code", "1.-<[_,-<[_,-<[_]]]")
                ));

                try {
                    final String json = jsonMapper.writeValueAsString(request);
                    LOG.debug("Sending tool call: %s", json);
                    send(json);
                } catch (final Exception e) {
                    LOG.error("Failed to send tool call: %s", e.getMessage());
                }
            }
            ...
            public void onMessage(final String message) {
                ...
                try {
                    final Map<String, Object> response = jsonMapper.readValue(message, Map.class);
                    final Object id = response.get("id");

                    if (Integer.valueOf(1).equals(id) && !initialized) {
                    ...
                    LOG.error("Failed to parse response: %s", e.getMessage());
                }
            }
        ...
        assertTrue(initLatch.await(10, TimeUnit.SECONDS), "Should initialize within 10 seconds");
        assertTrue(toolLatch.await(10, TimeUnit.SECONDS), "Should receive tool response within 10 seconds");

        assertNotNull(toolResponse.get(), "Should receive tool response");

        // Verify the response contains the result
        final Map<String, Object> response = jsonMapper.readValue(toolResponse.get(), Map.class);
        assertTrue(response.containsKey("result"), "Response should have result");
        ...
        LOG.info("Tool call result: %s", result);

        client.closeBlocking();
    }

    /**
     * Test MCP get_system_info tool.
     */
    @Test
    @Order(22)
    @TestCategory.Crud
    public void testMcpGetSystemInfoTool() throws Exception {
        final CountDownLatch initLatch = new CountDownLatch(1);
        final CountDownLatch toolLatch = new CountDownLatch(1);
        final AtomicReference<String> toolResponse = new AtomicReference<>();
        ...
        assertTrue(initLatch.await(10, TimeUnit.SECONDS), "Should initialize");
        assertTrue(toolLatch.await(10, TimeUnit.SECONDS), "Should receive tool response");
        ...
        final String infoText = (String) content.get(0).get("text");
        assertTrue(infoText.contains("Metatron System Information"),
                "System info should contain header");
        assertTrue(infoText.contains("Router"), "System info should mention Router");

        LOG.info("System info: %s", infoText);

        client.closeBlocking();
    }

    /**
     * Test MCP list_instructions tool.
     */
    @Test
    @Order(23)
    @TestCategory.Crud
    public void testMcpListInstructionsTool() throws Exception {
        final CountDownLatch initLatch = new CountDownLatch(1);
        final CountDownLatch toolLatch = new CountDownLatch(1);
        final AtomicReference<String> toolResponse = new AtomicReference<>();
